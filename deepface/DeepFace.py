...
def analyze(img_path, actions = ('emotion', 'age', 'gender', 'race') , models = None, enforce_detection = True, detector_backend = 'opencv', prog_bar = True, enable_multiple=False):
	...
	for j in global_pbar:
		...
		print(f"Face Detection Time : {int((time.time() - s) * 1000)}")
		...
		for index in pbar:
			...
			face_img, face_region, landmarks = faces[face_index]
			...
			if action == 'emotion':
				...
				print(f"{current_process()} emotion inference : {int((emotion_end - emotion_start) * 1000)}ms")
				...
				age_end = time.time()
				print(f"{current_process()} age inference : {int((age_end - age_start) * 1000)}ms")
				...
				print(f"{current_process()} gender inference : {int((gender_end - gender_start) * 1000)}ms")
...
def analyzes(img_path, actions = ('emotion', 'age', 'gender', 'race') , models = None, enforce_detection = True, detector_backend = 'retinaface'):
	...
	Returns:
		The function returns a list of dictionary.
		[
			{
				"region": {'x': 230, 'y': 120, 'w': 36, 'h': 45},
				"age": 28,
	...
	faces = functions.detect_faces(img=img_path, enforce_detection=enforce_detection, detector_backend=detector_backend)
	crop_faces_48 = [functions.reshape_face(img=face[0], region=face[1], target_size=(48, 48), grayscale=True)[0] for face in faces]
	crop_face_48_concat = np.concatenate(crop_faces_48, axis = 0)
	crop_faces_224 = [functions.reshape_face(img=face[0], region=face[1], target_size=(224, 224), grayscale=False)[0] for face in faces]		
	crop_face_224_concat = np.concatenate(crop_faces_224, axis = 0)
	...
	if faces: 				
		face_results = [{'region' : {parameter : face[1][j] for j, parameter in enumerate(['x', 'y', 'w', 'h'])}} for i, face in enumerate(faces)]
		for action in actions:
			if action == 'emotion':
				emotion_labels = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
				emotion_start = time.time()
				emotion_predictions = models['emotion'].predict(crop_face_48_concat)
				emotion_end = time.time()
				print(f"{current_process()} emotion inference : {int((emotion_end - emotion_start) * 1000)}ms")
				emotions = [emotion_labels[np.argmax(emotion)] for emotion in emotion_predictions]
				for i in range(len(face_results)):
					face_results[i]['emotion'] = emotions[i]


			elif action == 'age':
				age_start = time.time()
				age_predictions = models['age'].predict(crop_face_224_concat)
				age_end = time.time()
				print(f"{current_process()} age inference : {int((age_end - age_start) * 1000)}ms")
				ages = [int(Age.findApparentAge(age)) for age in age_predictions]
				for i in range(len(face_results)):
					face_results[i]['age'] = ages[i]

			elif action == 'gender':
				gender_start = time.time()
				gender_predictions = models['gender'].predict(crop_face_224_concat)
				gender_end = time.time()
				print(f"{current_process()} gender inference : {int((gender_end - gender_start) * 1000)}ms")
				genders = ["Man" if np.argmax(pred) == 1 else "Woman" for pred in gender_predictions]
				for i in range(len(face_results)):
					face_results[i]['gender'] = genders[i]

			elif action == 'race':
				race_predictions = models['race'].predict(crop_face_224_concat)
				race_labels = ['asian', 'indian', 'black', 'white', 'middle eastern', 'latino hispanic']
				races = [race_labels[np.argmax(race)] for race in race_predictions]
				for i in range(len(face_results)):
					face_results[i]['race'] = races[i]

	return face_results

def analyze_action(faces, action, model = None):
	...
	if faces: 				
		if action == 'emotion':
			emotion_labels = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
			emotion_start = time.time()
			emotion_predictions = model.predict(crop_face_48_concat)
			emotion_end = time.time()
			print(f"{current_process()} emotion inference : {int((emotion_end - emotion_start) * 1000)}ms")
			emotions = [emotion_labels[np.argmax(emotion)] for emotion in emotion_predictions]
			action_output = emotions

		elif action == 'age':
			age_start = time.time()
			age_predictions = model.predict(crop_face_224_concat)
			age_end = time.time()
			print(f"{current_process()} age inference : {int((age_end - age_start) * 1000)}ms")
			ages = [int(Age.findApparentAge(age)) for age in age_predictions]
			action_output = ages
			...
			gender_start = time.time()
			gender_predictions = model.predict(crop_face_224_concat)
			gender_end = time.time()
			print(f"{current_process()} gender inference : {int((gender_end - gender_start) * 1000)}ms")
			genders = ["Man" if np.argmax(pred) == 1 else "Woman" for pred in gender_predictions]
			action_output = genders

		elif action == 'race':
			race_predictions = model.predict(crop_face_224_concat)
			race_labels = ['asian', 'indian', 'black', 'white', 'middle eastern', 'latino hispanic']
			races = [race_labels[np.argmax(race)] for race in race_predictions]
			action_output = races
...
def analyze_results(face_result, actions = ('emotion', 'age', 'gender', 'race') , models = None, prog_bar = True, enable_multiple=False):
	actions = list(actions)
	if not models:
		models = {}
	...
	#facial attribute analysis
	face_img, face_region, landmarks = face_result
	face_response_obj['landmarks'] = landmarks
	face_response_obj["region"] = {}
	for i, parameter in enumerate(['x', 'y', 'w', 'h']):
		face_response_obj["region"][parameter] = int(face_region[i])  # int cast is for the exception - object of type 'float32' is not JSON serializable
	for action in actions:
		if action == 'emotion':
			emotion_labels = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
			img, region = functions.reshape_face(img=face_img.copy(), region=face_region.copy(), target_size=(48, 48), grayscale=True)
			...
			# face_response_obj["emotion"] = {}

			# for i in range(0, len(emotion_labels)):
			# 	emotion_label = emotion_labels[i]
			# 	emotion_prediction = 100 * emotion_predictions[i] / sum_of_predictions
			# 	face_response_obj["emotion"][emotion_label] = emotion_prediction

			face_response_obj["emotion"] = emotion_labels[np.argmax(emotion_predictions)]
			...
			face_response_obj["age"] = int(apparent_age) #int cast is for the exception - object of type 'float32' is not JSON serializable
			...
			face_response_obj["gender"] = gender
			...
			# face_response_obj["race"] = {}
			# for i in range(0, len(race_labels)):
			# 	race_label = race_labels[i]
			# 	race_prediction = 100 * race_predictions[i] / sum_of_predictions
			# 	face_response_obj["race"][race_label] = race_prediction

			face_response_obj["race"] = race_labels[np.argmax(race_predictions)]

	return face_response_obj

def consumer_analyze(face_result, actions = ('emotion', 'age', 'gender', 'race') , models = None):
	actions = list(actions)
	if not models:
		models = {}
	...
	#facial attribute analysis
	face_img, face_region = face_result
	face_response_obj["region"] = {}
	for i, parameter in enumerate(['x', 'y', 'w', 'h']):
		face_response_obj["region"][parameter] = int(face_region[i])  # int cast is for the exception - object of type 'float32' is not JSON serializable
	for action in actions:
...
def represents(faces, model = None, normalization = 'base', input_shape=(224, 224)):
	...
	#decide input shape
	# input_shape_x, input_shape_y = functions.find_input_shape(model)
	assert model is not None, "Model is none, input yout model "
	#detect and align
	imgs = functions.preprocess_faces(faces = faces
	...
	# custom_norm_imgs = [functions.normalize_input(img = img, normalization = normalization) for img in imgs]

	#---------------------------------

	#represent
	embeddings = [model.predict(img)[0].tolist() for img in imgs]
...
def detectFaces(img_path, enforce_detection=True, detector_backend='retinaface'):
	faces = functions.detect_faces(img=img_path, enforce_detection=enforce_detection, detector_backend=detector_backend)
	return faces
#---------------------------
#main
