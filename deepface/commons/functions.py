...
def detect_faces(img, enforce_detection=True, detector_backend='opencv', align=True):
    ...
    # for img, region, landmarks in faces:
...
def preprocess_face(img, target_size=(224, 224), grayscale = False, enforce_detection = True, detector_backend = 'opencv', return_region = False, align = True):
	...
	if return_region == True:
		...
		return img_pixels
def preprocess_faces(faces, target_size=(224, 224), grayscale = False, return_region = False):
	...
	results = []
	#--------------------------
	for img, region, landmark in faces:
		if img.shape[0] == 0 or img.shape[1] == 0:
		...
		if return_region == True:
			results.append((img_pixels, region))
		else:
			results.append(img_pixels)
